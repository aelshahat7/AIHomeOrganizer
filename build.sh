...
for tool in "$AAPT2" "$D8" "$APKSIGNER" "$ANDROID_JAR"; do
  if [ ! -e "$tool" ] || { [ ! -x "$tool" ] && [[ "$tool" != *.jar ]]; }; then echo "Missing required build component: $tool"; exit 1; fi
done
...
if [ ! -f "$KEYSTORE" ]; then
  keytool -genkeypair -v -keystore "$KEYSTORE" -storepass android -alias androiddebugkey -keypass android -keyalg RSA -keysize 2048 -validity 10000 -dname "CN=Android Debug,O=Android,C=US" >/dev/null 2>&1
...
"$AAPT2" link -o "$OUT/base.apk" -I "$ANDROID_JAR" --manifest "$PROJECT_DIR/AndroidManifest.xml" --java "$OUT/gen" "$OUT/res/resources.zip"
