...
public class MainActivity extends Activity {
    ...
    private void testRootBridge(){status.setText("Testing root + protected Launcher3 provider...");new Thread(()->{RootController rc=new RootController();RootController.Result r=rc.test();RootController.Result p=r.ok()?new RootLauncherController(rc).probe():r;runOnUiThread(()->{append("ROOT_TEST exit="+r.exitCode+" output="+safe(r.output)+"\n");append("LAUNCHER3_PROVIDER_TEST exit="+p.exitCode+" output="+safe(p.output)+"\n");status.setText(r.ok()?(p.ok()?"ROOT OK + Launcher3 Provider OK":"ROOT OK, Provider call failed"):("ROOT unavailable: "+r.output));refresh();});}).start();}
