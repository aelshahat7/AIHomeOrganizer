...
public final class GestureController {
    ...
    public void tap(int x, int y, Callback cb) {
        Path p = new Path(); p.moveTo(x, y); run(p, 0, 180, cb);
    }

    public void longPress(int x, int y, Callback cb) {
        Path p = new Path(); p.moveTo(x, y); p.lineTo(x + 1, y); run(p, 0, 750, cb);
    }
    ...
    public void safeDrag(int x, int y, int dx, int dy, Callback cb) {
        ...
        p.lineTo(x, y);
        p.lineTo(x + sx, y + sy);
        p.lineTo(x + Math.round(sx * .25f), y + Math.round(sy * .25f));
        run(p, 0, 1250, cb);
    }
    ...
    private void run(Path path, long start, long duration, Callback cb) {
        ...
        GestureDescription g = new GestureDescription.Builder()
                .addStroke(new GestureDescription.StrokeDescription(path, start, duration)).build();
        try {
